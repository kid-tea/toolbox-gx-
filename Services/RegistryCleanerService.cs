...
public class RegistryCleanerService : IRegistryCleanerService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        foreach (var issue in distinct)
        {
            issue.IsSelected = ShouldSelectByDefault(issue);
        }
        ...
        try
        {
            ...
            foreach (var ext in classesKey.GetSubKeyNames().Where(n => n.StartsWith(".")).Take(500))
            {
                try
                {
                    ...
                    if (!string.IsNullOrEmpty(exePath) && !PathTargetExists(exePath))
                    {
    ...
    private void FindEmptyKeys(RegistryKey key, string path, List<RegistryIssue> issues, int depth)
    {
        ...
        foreach (var subKeyName in key.GetSubKeyNames())
        {
            try
            {
                ...
                if (IsProtectedEmptyKeyPath(fullPath))
                    continue;
    ...
    private List<RegistryIssue> ScanStartupResidue()
    {
        ...
        var runKeys = new[]
        {
            ...
            (RegistryHive.LocalMachine, @"SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Run", "HKLM"),
    ...
    public RegistryBackup? CreateBackup(List<RegistryIssue> issues)
    {
        try
        {
            ...
            File.WriteAllText(filePath + ".sha256", hash, Encoding.UTF8);
    ...
    private string ExportRegistryKey(string registryPath)
    {
        try
        {
            ...
            AppendRegistryKeyRecursive(sb, key, regKeyPath);
    ...
    private string FormatRegValue(string name, object? value, RegistryValueKind kind)
    {
        ...
        return kind switch
        {
            ...
            _ => $"{quotedName}=\"{EscapeRegString(value.ToString() ?? "")}\""
        };
    }

    private void AppendRegistryKeyRecursive(StringBuilder sb, RegistryKey key, string regKeyPath)
    {
        ...
        foreach (var subKeyName in key.GetSubKeyNames())
        {
            try
            {
                ...
                AppendRegistryKeyRecursive(sb, subKey, $"{regKeyPath.TrimEnd(']')}\\{subKeyName}]");
            }
            catch (Exception ex)
            {
                _log.LogWarning($"导出注册表子键失败: {regKeyPath}\\{subKeyName}: {ex.Message}");
            }
        }
    }
    ...
    private bool FixSingleIssue(RegistryIssue issue)
    {
        ...
        if (parts[0].Equals("HKCR", StringComparison.OrdinalIgnoreCase) ||
            ...
            rootKey = Registry.CurrentUser;
                 ...
                 parts[0].Equals("HKEY_LOCAL_MACHINE", StringComparison.OrdinalIgnoreCase))
            rootKey = Registry.LocalMachine;
        else if (parts[0].Equals("HKLM_WOW64", StringComparison.OrdinalIgnoreCase))
            rootKey = Registry.LocalMachine;
        ...
        try
        {
            ...
            string fullPath = $"{parts[0]}\\{pathInReg}";
            if (issue.Category == RegistryScanCategory.EmptyKey && !IsRegistryKeyEmpty(parentKey, entryName))
                return false;

            if (IsProtectedDeletePath(fullPath))
                return false;
    ...
    public List<RegistryBackup> GetBackupHistory()
    {
        ...
        try
        {
            ...
            foreach (var file in files.OrderByDescending(f => f))
            {
                ...
                var hash = File.Exists(hashFile)
                    ? File.ReadAllText(hashFile, Encoding.UTF8).Trim()
                    : _config.ComputeHash(file);
    ...
    private static bool ShouldSelectByDefault(RegistryIssue issue)
    {
        return issue.Severity != IssueSeverity.High
            && issue.Category != RegistryScanCategory.EmptyKey
            && issue.Category != RegistryScanCategory.InvalidComOle;
    }

    private static bool IsProtectedEmptyKeyPath(string registryPath)
    {
        string path = NormalizeRegistryPathForCompare(registryPath);
        return path.Equals("HKCU\\SOFTWARE", StringComparison.OrdinalIgnoreCase)
            || path.Equals("HKCU\\SOFTWARE\\MICROSOFT", StringComparison.OrdinalIgnoreCase)
            || path.StartsWith("HKCU\\SOFTWARE\\MICROSOFT\\", StringComparison.OrdinalIgnoreCase)
            || path.Equals("HKCU\\SOFTWARE\\CLASSES", StringComparison.OrdinalIgnoreCase)
            || path.StartsWith("HKCU\\SOFTWARE\\CLASSES\\", StringComparison.OrdinalIgnoreCase)
            || path.StartsWith("HKLM\\SOFTWARE\\MICROSOFT\\WINDOWS\\", StringComparison.OrdinalIgnoreCase);
    }

    private static bool IsProtectedDeletePath(string registryPath)
    {
        string path = NormalizeRegistryPathForCompare(registryPath);
        return path.Equals("HKLM\\SOFTWARE", StringComparison.OrdinalIgnoreCase)
            || path.Equals("HKCU\\SOFTWARE", StringComparison.OrdinalIgnoreCase)
            || path.Equals("HKCR\\CLSID", StringComparison.OrdinalIgnoreCase)
            || path.StartsWith("HKLM\\SYSTEM\\", StringComparison.OrdinalIgnoreCase);
    }

    private static string NormalizeRegistryPathForCompare(string registryPath)
    {
        var path = registryPath.Replace('/', '\\').Trim();
        if (path.StartsWith("HKEY_CURRENT_USER\\", StringComparison.OrdinalIgnoreCase))
            path = "HKCU\\" + path["HKEY_CURRENT_USER\\".Length..];
        else if (path.StartsWith("HKEY_LOCAL_MACHINE\\", StringComparison.OrdinalIgnoreCase))
            path = "HKLM\\" + path["HKEY_LOCAL_MACHINE\\".Length..];
        else if (path.StartsWith("HKEY_CLASSES_ROOT\\", StringComparison.OrdinalIgnoreCase))
            path = "HKCR\\" + path["HKEY_CLASSES_ROOT\\".Length..];
        else if (path.StartsWith("HKLM_WOW64\\", StringComparison.OrdinalIgnoreCase))
            path = "HKLM\\" + path["HKLM_WOW64\\".Length..];

        return path.ToUpperInvariant();
    }

    private bool IsIssueStillValid(RegistryIssue issue)
    {
        try
        {
            if (issue.RegistryPath.StartsWith("ShellLink:", StringComparison.OrdinalIgnoreCase))
            {
                var shortcutPath = issue.RegistryPath[10..];
            ...
            {
                RegistryScanCategory.StartupResidue => IsRegistryValueTargetMissing(issue.RegistryPath),
                RegistryScanCategory.InvalidFileAssociation => IsRegistryDefaultValueTargetMissing(issue.RegistryPath),
                RegistryScanCategory.UninstallResidue => IsUninstallResidueStillValid(issue.RegistryPath),
                RegistryScanCategory.EmptyKey => IsRegistryKeyEmpty(issue.RegistryPath) && !IsProtectedEmptyKeyPath(issue.RegistryPath),
                RegistryScanCategory.InvalidComOle => IsComRegistrationStillInvalid(issue.RegistryPath),
                _ => true
            };
        }
        catch (Exception ex)
        {
            _log.LogWarning($"复验注册表问题失败: {issue.RegistryPath}: {ex.Message}");
            return false;
        }
    }

    private bool IsRegistryValueTargetMissing(string registryPath)
    {
        if (!TryOpenParentRegistryKey(registryPath, writable: false, out var parentKey, out var valueName))
            return false;

        using (parentKey)
        {
            var value = parentKey.GetValue(valueName)?.ToString();
            var exePath = ExtractExePath(value ?? "");
            return !string.IsNullOrWhiteSpace(exePath) && !PathTargetExists(exePath);
        }
    }

    private bool IsRegistryDefaultValueTargetMissing(string registryPath)
    {
        if (!TryOpenRegistryKey(registryPath, writable: false, out var key))
            return false;
    ...
    private bool IsUninstallResidueStillValid(string registryPath)
    {
        if (!TryOpenRegistryKey(registryPath, writable: false, out var key))
            return false;
        ...
        using (key)
        {
            ...
            if (!string.IsNullOrWhiteSpace(installLocation) && !Directory.Exists(Environment.ExpandEnvironmentVariables(installLocation)))
                return true;
...
#pragma warning disable CS8602
    private bool IsComRegistrationStillInvalid(string? registryPath)
    {
        if (string.IsNullOrWhiteSpace(registryPath))
            return false;
        ...
        using (var clsidKey = key!)
        {
            using var inprocKey = clsidKey.OpenSubKey("InprocServer32", writable: false);
            var dllPath = inprocKey?.GetValue("")?.ToString();
            if (!string.IsNullOrWhiteSpace(dllPath))
            {
                dllPath = ResolveExecutableCandidate(Environment.ExpandEnvironmentVariables(dllPath));
                if (!PathTargetExists(dllPath))
                    return true;
            }

            using var localServerKey = clsidKey.OpenSubKey("LocalServer32", writable: false);
            var serverPath = localServerKey?.GetValue("")?.ToString();
            var exePath = ExtractExePath(serverPath ?? "");
            return !string.IsNullOrWhiteSpace(exePath) && !PathTargetExists(exePath);
        }
    }
#pragma warning restore CS8602

    private bool IsRegistryKeyEmpty(string registryPath)
    {
        if (!TryOpenRegistryKey(registryPath, writable: false, out var key))
            return false;
    ...
    private static bool IsRegistryKeyEmpty(RegistryKey parentKey, string subKeyName)
    {
        using var key = parentKey.OpenSubKey(subKeyName, writable: false);
        return key != null && key.ValueCount == 0 && key.SubKeyCount == 0;
    }

    private static bool TryOpenParentRegistryKey(
        string registryPath,
        bool writable,
        out RegistryKey? parentKey,
        out string entryName)
    {
        parentKey = null;
        entryName = "";

        var normalized = NormalizeRegistryPathForOpen(registryPath);
        var lastSep = normalized.LastIndexOf('\\');
        if (lastSep <= 0) return false;

        entryName = normalized[(lastSep + 1)..];
        return TryOpenRegistryKey(normalized[..lastSep], writable, out parentKey);
    }

    private static bool TryOpenRegistryKey(string registryPath, bool writable, out RegistryKey? key)
    {
        key = null;
        if (string.IsNullOrWhiteSpace(registryPath))
            return false;
        ...
        key = rootKey.OpenSubKey(parts[1], writable);
        return key != null;
    }

    private static string NormalizeRegistryPathForOpen(string registryPath)
    {
        var normalized = registryPath.Replace('/', '\\').Trim();
        if (normalized.StartsWith("HKLM_WOW64\\", StringComparison.OrdinalIgnoreCase))
            normalized = "HKLM\\" + normalized["HKLM_WOW64\\".Length..];

        return normalized;
    }

    private static bool PathTargetExists(string path)
    {
        if (string.IsNullOrWhiteSpace(path))
            return false;

        var candidate = ResolveExecutableCandidate(path);
    ...
    private static string ExtractExePath(string command)
    {
        ...
        if (command.StartsWith("\""))
        {
            ...
            if (end > 0) return ResolveExecutableCandidate(command[1..end]);
        }

        var extensions = new[] { ".exe", ".dll", ".ocx", ".cpl", ".bat", ".cmd", ".msi" };
        foreach (var extension in extensions)
        {
            int extensionIndex = command.IndexOf(extension, StringComparison.OrdinalIgnoreCase);
            if (extensionIndex >= 0)
                return ResolveExecutableCandidate(command[..(extensionIndex + extension.Length)]);
        }

        int space = command.IndexOf(' ');
        string path = space > 0 ? command[..space] : command;
        return ResolveExecutableCandidate(path);
    }
