...
public partial class App : Application
{
    ...
    protected override void OnStartup(StartupEventArgs e)
    {
        ...
        // 允许从资源管理器拖拽文件到应用窗口（解决普通权限 Explorer → 窗口拖放被拦截）
    ...
    /// <summary>
    /// 允许 Explorer 向当前应用窗口发送文件拖放消息
    /// </summary>
    private static void EnableExplorerDragDrop()
    {
        try
        {
            Native.NativeMethods.ChangeWindowMessageFilter(0x0049, 1);
            Native.NativeMethods.ChangeWindowMessageFilter(0x0233, 1);
            Native.NativeMethods.ChangeWindowMessageFilter(0x0234, 1);
        }
        catch
        {
            // 某些系统/权限下调用失败时忽略，不阻止启动
        }
    }
