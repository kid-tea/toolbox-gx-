...
public interface ICleanupService
{
    ...
    Task ScanAllAsync(IProgress<string> progress, CancellationToken token);
    ...
    Task<CleanupResult> CleanAsync(IEnumerable<CleanupItem> items, IProgress<(string, int)> progress, CancellationToken token);
...
public class CleanupService : ICleanupService
{
    ...
    /// </summary>
    public async Task ScanAllAsync(IProgress<string> progress, CancellationToken token)
    {
        // 重置所有项目的统计数据
    ...
    /// </summary>
    private void ScanItemSize(CleanupItem item, CancellationToken token)
    {
        ...
        foreach (var path in paths)
        {
            ...
            if (trimmedPath.Contains("*"))
            {
                ...
                foreach (var expandedPath in expandedPaths)
                {
                    if (Directory.Exists(expandedPath))
                        ScanDirectory(expandedPath, item, ref totalSize, ref fileCount, token);
                }
                continue;
            }
            ...
            try
            {
                ScanDirectory(trimmedPath, item, ref totalSize, ref fileCount, token);
            }
    ...
    private void ScanDirectory(string directoryPath, CleanupItem item,
        ref long totalSize, ref int fileCount, CancellationToken token)
    {
