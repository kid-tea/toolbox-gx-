...
public partial class BatchRenameViewModel : ViewModelBase
{
    ...
    private void RefreshPreview()
    {
        ...
        try
        {
            foreach (var item in FileItems)
            {
                ...
                // 安全检测
                CheckPreviewSafety(item, newName);
    ...
    /// 应用单个规则到文件名（不含扩展名）
    /// </summary>
    private static string ApplyRule(string name, RenameRule rule)
    {
        try
        {
            return rule.RuleType switch
            {
                ...
                RenameRuleType.Numbering => name, // 序号在批量应用时处理
    ...
    /// 检查预览结果的安全性并设置警告
    /// </summary>
    private void CheckPreviewSafety(RenameFileItem item, string newNameWithoutExt)
    {
        item.Warning = RenameWarningType.None;
        item.WarningMessage = "";
        item.IsErrorWarning = false;
        ...
        // 检测3：非法字符
        string fullNewName = newNameWithoutExt + item.Extension;
