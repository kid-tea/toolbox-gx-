...
public partial class CleanupViewModel : ViewModelBase
{
    ...
    private async Task ScanAsync()
    {
        ...
        try
        {
            ...
            await _cleanupService.ScanAllAsync(progress, token);
    ...
    private async Task CleanAsync()
    {
        ...
        try
        {
            ...
            var result = await _cleanupService.CleanAsync(selectedItems, progress, token);
    ...
    private async Task ExecuteScheduledCleanupAsync(CancellationToken token)
    {
        ...
        if (!IsScanned)
        {
            try
            {
                ...
                await _cleanupService.ScanAllAsync(scanProgress, token);
            }
            catch (Exception ex)
            {
