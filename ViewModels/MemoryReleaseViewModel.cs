...
public partial class MemoryReleaseViewModel : ViewModelBase
{
    ...
    private async Task ReleaseStandbyListAsync()
    {
        ...
        if (!ConfirmationHelper.RequestL1(
            ...
            "副作用：之后首次打开常用软件、文件或游戏加载资源时可能变慢，因为 Windows 需要重新建立缓存。\n\n" +
            "建议：只有内存压力明显、游戏/大型软件卡顿或待机缓存异常偏高时再点；一般不需要频繁使用，最多按需每几小时一次。\n\n" +
            "是否继续？"))
            return;
        ...
        try
        {
            ...
            await Task.Run(() =>
            {
                ...
                try
                {
                    ...
                    if (result != 0)
                        throw CreateNtSetSystemInformationException(result);
                }
                finally
                {
    ...
    private async Task ReleaseSystemCacheAsync()
    {
        ...
        if (!ConfirmationHelper.RequestL1(
            "此操作调用系统未公开 API (NtSetSystemInformation)，可能在 Windows 更新后失效。\n\n" +
            "操作本身是安全的（仅清空缓存，程序会自动重建），不会影响正在运行的程序和数据。\n\n" +
            "副作用：短时间内磁盘读取可能增加，系统、资源管理器或大型程序可能出现短暂卡顿。\n\n" +
            "建议：这是更重的缓存刷新，只在系统缓存异常占用、释放待机列表无效或系统明显卡顿时再点；通常一天内很少需要使用。\n\n" +
            "是否继续？"))
            return;
        ...
        try
        {
            ...
            await Task.Run(() =>
            {
                var command = new NativeMethods.SYSTEM_MEMORY_LIST_COMMAND
                {
                    Command = NativeMethods.MEMORY_LIST_COMMAND.MemoryFlushModifiedList
                };
    ...
    private static void EnablePrivilege(string privilegeName)
    {
        ...
        try
        {
            var access = NativeMethods.TOKEN_ADJUST_PRIVILEGES | NativeMethods.TOKEN_QUERY;
            if (!NativeMethods.OpenProcessToken(NativeMethods.GetCurrentProcess(), access, out tokenHandle))
                throw new InvalidOperationException($"OpenProcessToken 失败: {Marshal.GetLastWin32Error()}");

            if (!NativeMethods.LookupPrivilegeValue(null, privilegeName, out var luid))
                throw new InvalidOperationException($"LookupPrivilegeValue({privilegeName}) 失败: {Marshal.GetLastWin32Error()}");
            ...
            if (!NativeMethods.AdjustTokenPrivileges(tokenHandle, false, ref privileges, 0, IntPtr.Zero, IntPtr.Zero))
                throw new InvalidOperationException($"AdjustTokenPrivileges 失败: {Marshal.GetLastWin32Error()}");

            var error = Marshal.GetLastWin32Error();
            if (error != 0)
                throw new InvalidOperationException($"{privilegeName} 未授予当前进程: {error}");
        }
        finally
        {
    ...
    private static InvalidOperationException CreateNtSetSystemInformationException(NTSTATUS result)
    {
        uint status = unchecked((uint)result);
        return status switch
        {
            0xC0000061 => new InvalidOperationException(
                "权限不足：当前进程没有 SeProfileSingleProcessPrivilege，请以管理员身份重新启动工具箱。"),
            _ => new InvalidOperationException($"NtSetSystemInformation 返回 0x{status:X8}")
        };
    }
