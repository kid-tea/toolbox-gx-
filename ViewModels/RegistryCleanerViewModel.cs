...
public partial class RegistryCleanerViewModel : ViewModelBase
{
    ...
    [RelayCommand]
    private void SelectAllScanCategories()
    {
        foreach (var category in ScanCategories)
        {
            category.IsSelected = true;
        }

        StatusMessage = "已全选扫描分类，包含空注册表键和无效 COM/OLE";
    }

    [RelayCommand]
    private void SelectSafeScanCategories()
    {
        foreach (var category in ScanCategories)
        {
            category.IsSelected = category.Category != RegistryScanCategory.EmptyKey
                && category.Category != RegistryScanCategory.InvalidComOle;
        }

        StatusMessage = "已恢复安全默认分类，空注册表键和无效 COM/OLE 需手动选择";
    }
    ...
    private void DeleteBackup(RegistryBackup? backup)
    {
        ...
        try
        {
            if (File.Exists(backup.FilePath))
            {
                ...
                var hashFile = backup.FilePath + ".sha256";
